...
gj.grid.methods = {
    ...
    updateRow: function ($grid, id, record) {
        ...
        if ($.isArray(data.dataSource)) {
            ...
            position = id; //GTSB - Removed getRecVPosById call due to timing issue on watch calls
    ...
    removeRow: function ($grid, id, get_all) {
        var position,
            data = $grid.data(),
            $row = gj.grid.methods.getRowById($grid, id);
...
gj.grid.widget = function ($grid, jsConfig) {
     ...
     * </script>
     */
    self.removeRow = function (id, get_all) {
        return methods.removeRow(this, id, get_all);
    };
